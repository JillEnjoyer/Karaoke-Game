...
signal request_delete(segment_id) # request to delete the segment
signal request_copy(segment_id) # request to copy the segment
signal request_go_to(segment_id, at_time) # request to go to the segment
signal position_changed(segment_id, new_offset) # signal for segment position change
signal size_changed(segment_id, new_size) # signal for segment size change
...
func _on_left_handle_gui_input(event: InputEvent) -> void:
	...
	if dragging_left and event is InputEventMouseMotion:
		...
		var new_left_cutoff = clamp(initial_left_cutoff + (delta / size.x) * total_duration, 0, total_duration - right_cutoff)
...
func _on_drag_btn_gui_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		wrapper.size.x = wrapper.size.x + 1000.0 # если правая точка ближе чем в 1000px от правого края - расширяем обертку. После окончания сжимаем
		if is_dragging_track and event is InputEventMouseMotion:
			...
			emit_signal("position_changed", segment_id, new_offset)
		...
		func(id):
			match id:
				0:
					...
					emit_signal("request_go_to", segment_id)
				1:
					var cut_time = 50 # start_time + end_time to get size and pointer position to find a cut place
					emit_signal("request_split", segment_id, cut_time)
				2:
					# copying segment and pasting it at right
					emit_signal("request_copy", segment_id)
				3:
					emit_signal("request_delete", segment_id)
	)
