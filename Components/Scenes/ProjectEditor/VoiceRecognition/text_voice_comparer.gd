...
@onready var input_text_edit = $TextEdit
@onready var output_text_edit = $CodeEdit
@onready var path_text = $HBoxContainer/RichTextLabel ## need to have him editable
...
var vosk_model_path = ProjectSettings.globalize_path("res://Extensions/Models/[ENG]vosk-model-0.22-lgraph") ## TODO: Get path from root folder in PreferencesData

var user_reference_text: String = ""
var vosk_recognized_text: String = ""
var vosk_recognized_text_final: String = ""


func import_text_data_from_controller(user_text: String, vosk_text: String) -> void:
	user_reference_text = user_text
	vosk_recognized_text = vosk_text
...
func _on_process_finished(result, output):
	if result == 0:
		output_text_edit.text = str(output[0])
		Debugger.info("Ready!")
		#var res: Array = TextComparisor.align_text_with_vosk(input_text_edit.text, output_text_edit.text)
		#output_text_edit.text = str(res)
	else:
