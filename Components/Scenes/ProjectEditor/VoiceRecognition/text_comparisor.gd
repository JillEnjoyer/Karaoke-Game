...
static func align_text_with_vosk(original_text: String, vosk_raw_output: String) -> Array:
	...
	if json_start == -1:
		push_error("JSON is not found in Vosk output!")
		return []
	...
	if error != OK:
		push_error("Error in parsing JSON: ", json.get_error_message(), " on line ", json.get_error_line())
		return []
	...
	if vosk_words.size() == 0:
		push_warning("Vosk recognized nothing.")
