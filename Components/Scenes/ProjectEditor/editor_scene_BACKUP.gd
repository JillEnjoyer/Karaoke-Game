...
func _ready() -> void:
	...
	#file_dialog_init()
	#media_player.size = player_scene.size
	media_player.editor_mode = true
	media_player.get_node("PlaybackManager/VideoManager/TextureRect").texture = preload("res://icon.svg") ## change with no media icon :(
...
func files_selected(paths: PackedStringArray):
	...
	functionality_init(chosen_files)


func _on_new_project_btn_pressed() -> void:
	Debugger.debug("Create new song project...")
	var new_project: Control = UIManager.show_ui("NewProject", self)
	new_project.connect("new_project_closed", Callable(self, "new_project_created"))
func new_project_created(result: bool, chosen_files: Array) -> void:
	if result:
		functionality_init(chosen_files)
...
func add_files_to_timeline(chosen_files: Array) -> void:
...
func save_project() -> Dictionary:
	...
	for wrapper in wrappers:
		...
		var char_name = wrapper.node_name # "Alastor", "Original"
