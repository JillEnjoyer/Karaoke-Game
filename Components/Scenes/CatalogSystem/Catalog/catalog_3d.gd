...
func _ready() -> void:
	...
	await get_tree().process_frame # Let the scene load completely
	scan_folder()
...
func scan_folder() -> void:
	...
	_clear_current_objects(false)  # Clear without animation on first load
